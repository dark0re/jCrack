...
public final class Sha512Crypt
{
  ...
  static private final int ROUNDS_MIN = 500;
  ...
  public static final String Sha512_crypt(String keyStr, String saltStr, int roundsCount)
  {
	...
	if (saltStr.length() > SALT_LEN_MAX)
	  { 
	    saltEnd = saltStr.indexOf('$'); //used to get the correct salt (length of salt)
	    //System.out.println("Using salt index: " + saltEnd); 
		saltStr = saltStr.substring(0, saltEnd);
	  }
      }
    else
      {
  ...
  public final boolean verifyPassword(String plaintextPass, String sha512CryptText)
  {
    if (sha512CryptText.startsWith("$6$"))
      {
    	//System.out.println(sha512CryptText);
